...
void AmclNode::runFromBag(const std::string &in_bag_fn)
{
  ...
  ROS_INFO("Final location %.3f, %.3f, %.3f with stamp=%f, w_sum_sum=%f",
...
bool AmclNode::setResetFlagCallback(std_srvs::SetBool::Request &req,
                                    std_srvs::SetBool::Response &res)
{
  do_reset_ = req.data;
  // ROS_INFO("do_reset: %d \n", (int)do_reset_);
 ...
 */
    ...
    if (pf_->is_done_reset)
    {
      ...
      notify.data = pf_->is_done_reset;
      reset_notify_pub_.publish(notify);
    }
